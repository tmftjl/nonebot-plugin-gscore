...
Despite the name, the `v4-nonebot2` is a thin **connector plugin** (`nonebot-plugin-gscore`) 
that bridges a NoneBot2 instance to a separate server called **gsuid_core** (a.k.a. SayuCore). 
All actual feature/command logic lives in gsuid_core plugins, *not here*. 
This repo only does two things:
...
- **Plugin entrypoint** — root `__init__.py` calls `load_plugins(...)` to load the `gscore/`
  subpackage as the actual NoneBot2 plugin.
- **Event ingestion** — `gscore/__init__.py`. Registers matchers (`on_message`, `on_notice`,
...
- **WebSocket client** — `gscore/client.py`. `GsClient` is a **singleton** (`__new__` returns
...
- **Wire protocol** — `gscore/models.py`. `msgspec.Struct` types (`Message`, `MessageReceive`,
...
- Commits use **gitmoji**-style prefixes (`🎨`, `✨`, `🔖`, `⬆️`)
